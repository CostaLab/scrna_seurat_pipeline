...
## Clinical Metadata
```{r clinical_meta_plots, results="asis", echo=FALSE}
clinical_plots <- list.files(
  params$report_plots_folder_png,
  pattern = "^qc_clinical_sampleplot_.*\\.png$",
  full.names = TRUE
)
...
if (length(clinical_plots) > 0) {
  ...
  clinical_tbl <- tryCatch(
    load_object(file.path(params$report_tables_folder,
                          "clinical_meta_summary.RDS")),
