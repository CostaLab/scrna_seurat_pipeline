...
```{r prefilter_vlnplot, out.width="100%", echo=FALSE}
...
## Clinical Metadata
```{r clinical_meta_plots, results='asis', out.width="100%", echo=FALSE}
clinical_plots <- list.files(params$report_plots_folder_png,
  pattern = "^clinical_barplot_.*\\.png$", full.names = TRUE)
if (length(clinical_plots) > 0) {
  clinical_tbl <- tryCatch(
    load_object(file.path(params$report_tables_folder, "clinical_meta_summary.RDS")),
    error = function(e) NULL)
  if (!is.null(clinical_tbl)) {
    clinical_tbl %>%
      kable(caption = "Clinical metadata per sample", align = 'c') %>%
      kable_styling(bootstrap_options = c("striped", "hover")) %>%
      print()
  }
  for (p in sort(clinical_plots)) {
    cat("\n\n")
    print(knitr::include_graphics(p))
  }
} else {
  cat("No clinical metadata available.\n")
}
```
...
```{r high_var_genes, out.width="100%", echo=FALSE}
knitr::include_graphics(file.path(params$report_plots_folder_png,"high_var_genes.png"))
```

## Cellcycle scaling

### before
```{r cellcycle_scaling_before, out.width="100%", echo=FALSE}
knitr::include_graphics(file.path(params$report_plots_folder_png,"cellcycle_scaling_before.png"))
```

### after
```{r cellcycle_scaling_after, out.width="100%", echo=FALSE}
knitr::include_graphics(file.path(params$report_plots_folder_png,"cellcycle_scaling_after.png"))
```
