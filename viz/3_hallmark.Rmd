---
title: "hallmark"
author: "Mingbo Cheng"
date: "`r format(Sys.time(), '%d %B, %Y')`"
output:
  html_document:
    toc: yes
    toc_depth: 3
  pdf_document:
    toc: yes
    toc_depth: '3'
params:
    cluster: "clusters to analyse"
    project: "name/tag of the project"
    savedir: "directory where rds files were saved"
    funcs: "analysis steps to be performed"
    report_data_folder: "folder with the output tables"
    report_tables_folder: "folder with the table elements"
    report_plots_folder: "folder with the plot elements"
    report_plots_folder_png: "folder with the png plot elements"
    report_plots_folder_pdf: "folder with the pdf plot elements"
---

## Cluster: `r colorize(params$cluster,"red")`

## hallmark up analysis
```{r heatmap_hallmark_up , out.width="100%", echo=FALSE}
fname=file.path(
  params$report_plots_folder_png,
  paste0("heatmap_hallmark-up_cluster-",params$cluster,".png")
)
if(file.exists(fname)) knitr::include_graphics(fname)
```

```{r barplot_hallmark_up, out.width="100%", echo=FALSE}
### up genes top 10
fname=file.path(
  params$report_plots_folder_png,
  paste0("hallmark_up_genes_barplot_cluster-",params$cluster,".png")
)
if(file.exists(fname)) knitr::include_graphics(fname)
```

## hallmark down analysis
```{r heatmap_hallmark_down, out.width="100%", echo=FALSE}
fname=file.path(
  params$report_plots_folder_png,
  paste0("heatmap_hallmark-down_cluster-",params$cluster,".png")
)
if(file.exists(fname)) knitr::include_graphics(fname)
```

```{r barplot_hallmark_down, out.width="100%", echo=FALSE}
### down genes top 10
fname=file.path(
  params$report_plots_folder_png,
  paste0("hallmark_down_genes_barplot_cluster-",params$cluster,".png")
)
if(file.exists(fname)) knitr::include_graphics(fname)
```
