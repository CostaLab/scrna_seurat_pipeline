Genesets_stageVS_elements <- function(scrna){
  ...
  for(nm in scrna@tools$genesets){
    ...
    col_def <- ggsci_pal(option = "futurama")(
      length(unique(scrna@meta.data[, "stage"]))
    )
    plt <- ggplot(
        df.s,
        aes(x=value, y=Cluster, color=stage, point_color=stage, fill=stage)
      ) +
      geom_density_ridges(
        jittered_points=FALSE, scale = .95, rel_min_height = .01, alpha=0.5
      ) +
      ...
      guides(
        fill = guide_legend(override.aes = list(fill = col_def,
