...
if(fisher_cluster_name %ni% names(scrna@tools)){
    stop(glue("ERORR:fishertest hasn't been calculate for cluster{cluster_use}\n Please run [scrna_fishertest_clusters]!!!"))
}
...
for(nm in nms){
    ...
    plist[[nm]] <- DimPlot(scrna, reduction = "harmony_UMAP", group.by =  paste0("integrated_snn_res.", nm),
...
DimPlot(scrna, reduction = "harmony_UMAP", group.by = "name", cols=colours)
DimPlot(scrna, reduction = "harmony_UMAP", group.by = cluster_use, label=T, label.size=8)
...
### Cell cycle phase 
```{r fig.width=9, fig.height=7}
DimPlot(scrna, reduction = "harmony_UMAP", group.by = "Phase")
```
