...
if(cluster_use %ni% names(scrna@meta.data)){
  stop(glue("ERORR:There's no this {cluster_use} slot, please check!!!"))
}

if(fisher_cluster_name %ni% names(scrna@tools)){
  stop(glue("ERORR:fishertest hasn't been calculated for cluster {cluster_use}\n Please run [scrna_fishertest_clusters]!!!"))
}
...
for(nm in nms){
    ...
    plist[[nm]] <- DimPlot(scrna, reduction = "DEFAULT_UMAP", group.by =  paste0("integrated_snn_res.", nm),
...
DimPlot(scrna, reduction = "DEFAULT_UMAP", group.by = "name", cols=colours)
DimPlot(scrna, reduction = "DEFAULT_UMAP", group.by = cluster_use, label=T, label.size=8)
...
### Cell cycle phase 
```{r fig.width=9, fig.height=7}
DimPlot(scrna, reduction = "DEFAULT_UMAP", group.by = "Phase")
```
...
FeaturePlot(scrna, features = c("percent.mt", "percent.ribo", "nCount_RNA", "nFeature_RNA"), 
                   ...
                   ncol = 2)
FeaturePlot(scrna, features = c("CC.Difference","G1.Score", "S.Score", "G2M.Score"),
                   reduction = "DEFAULT_UMAP",
                   cols = c("lightgrey", "red"),
                   ncol = 2)
```
...
### Violin Plot 
```{r fig.width=9, fig.height=7}
VlnPlot(scrna, features = c("percent.mt", "percent.ribo", "nCount_RNA", "nFeature_RNA", "CC.Difference", "G1.Score","S.Score", "G2M.Score"),
