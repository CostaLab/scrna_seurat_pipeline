...
DimPlot(scrna, reduction = "INTE_UMAP", group.by = "name")
...
if(length(table(scrna$stage))<=2){
    ...
    aodds <- paste0("odds.ratio_", nm) 
    p <- ggplot(data=df, aes_string(x = "Cluster", y = aodds, fill = "Cluster")) +
      geom_bar(stat="identity") +    
      coord_flip() +
      scale_y_continuous(trans = shift_trans(1)) + geom_text(data = df, 
                  aes_string("Cluster", 1, label = sprintf("pval.adjust_%s", nm)), 
                  position = "identity",
                  size=4) + 
      ggtitle(sprintf("%s vs %s (odds ratio > 0 means more %s", nm, nm2, nm))+ 
      xlab("Cluster") +
      ylab("odds ratio")  
    print(p)
    ...
    for (i in 1:n){
        ...
        geom_text(data = df, 
                  aes_string("Cluster", 1, label = sprintf("pval.adjust_%s", nm)), 
                  position = "identity",
                  size=4) + ggtitle(sprintf("%s vs %s (odds ratio > 0 means more %s", nm, nm2, nm))+ 
                  xlab("Cluster") +
                  ylab("odds ratio")  
      print(p)
    }
...
p <- DimPlot(object = scrna, pt.size = 0.2, do.label = FALSE, 
	         group.by = "MCA_annotate", label.size = 1, do.hover=TRUE,
			 data.hover = "MCA_annotate") +
...
p <- DimPlot(object = scrna, pt.size = 0.2, do.label = FALSE,
	         group.by = "external_annotation", label.size = 1, do.hover=TRUE,
			 data.hover = "external_annotation") +
	 theme(legend.position = "right", 
		   legend.title = element_text(colour="blue", size=4, face="bold"),
	       legend.text = element_text(size = 7))
