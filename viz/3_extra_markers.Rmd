---
title: "extra marker genes imputated by MAGIC"
date: '`r format(Sys.Date(), "%Y-%B-%d")`'
output:
  html_document:
    toc: yes
    toc_depth: 3
  pdf_document:
    toc: yes
    toc_depth: '3'
params:
    scrna: 'scrna object'
    cluster: "clusters to analyse"
    cluster_info: "cluster info"
    project: "name/tag of the project"
    savedir: "directory where rds files were saved"
    funcs: "analysis steps to be performed"
    report_data_folder: "folder with the output tables"
    report_tables_folder: "folder with the table elements"
    report_plots_folder: "folder with the plot elements"
    report_plots_folder_png: "folder with the png plot elements"
    report_plots_folder_pdf: "folder with the pdf plot elements"
    author: "author of document"
author: '`r params$author`'
---

```{r extra_ext_markers_setup, include=FALSE}
knitr::opts_chunk$set(warning = F, message = F, echo = T)
celltype_names = load_object(file.path(params$report_tables_folder, "extra_ext_annot_celltype_names.RDS"))
sanitize_celltype_name <- function(x) gsub("[^A-Za-z0-9_.-]", "_", x)
```

## Extra markers
```{r extra_ext_markers, results="asis", out.width="100%", echo=FALSE}
for (a_celltype in celltype_names){
  safe_ct <- sanitize_celltype_name(a_celltype)
  cat('\n')
  cat("  \n###",  glue::glue("{a_celltype}"),"   \n")
  cat('\n')
  flist = grep(
    paste0("extra_extmarkers_inte_umap_featureplot_",safe_ct),
    list.files(
      params$report_plots_folder_png,
      full.names=TRUE
    ),
    value=TRUE,
    fixed=TRUE
  )
  for(i in flist){
    cat("![](",i,")")
  }

  fname = file.path(
    params$report_plots_folder_png,
    paste0("extra_extmarkers_dotplot_",safe_ct,"_groupby-",params$cluster,".png")
  )
  if(file.exists(fname)) cat("![](",fname,")")

  flist = grep(
    paste0("extra_extmarkers_vlnplot_",safe_ct),
    list.files(
      params$report_plots_folder_png,
      full.names=TRUE
    ),
    value=TRUE,
    fixed=TRUE
  )
  for(i in flist){
    cat("![](",i,")")
  }
}
```
