Genesets_1v1_elements <- function(scrna){
  cluster_use <- cluster
  ## Ridges plots
  for(nm in scrna@tools$genesets){
    ...
    col_def <- ggsci_pal(option = "futurama")(length(unique(scrna@meta.data[, "name"])))
    plt <- ggplot(df.s, aes(x=value, y=Cluster, color=name, point_color=name, fill=name)) +
  ...
  scrna$name <- as.character(scrna$name)
