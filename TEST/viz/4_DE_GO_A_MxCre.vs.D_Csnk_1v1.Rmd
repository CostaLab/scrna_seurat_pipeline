...
## Plot A_MxCre vs D_Csnk
```{r heatmap_de, out.width="100%", echo=FALSE}
flist = stringr::str_sort(grep(
  glue("top10-deg-vs_A_MxCre.vs.D_Csnk_cluster-{params$cluster}_p"),
  list.files(
...
flist = stringr::str_sort(grep(
  glue("volcanoplot_deg_vs_A_MxCre.vs.D_Csnk_cluster-{params$cluster}_id-"),
  list.files(
    params$report_plots_folder_png,
    full.names=TRUE
  ),
...
fname=file.path(
  params$report_plots_folder_png,
  glue("go_up_genes_heatmap_vs_A_MxCre.vs.D_Csnk_cluster-{params$cluster}.png")
)
...
### up genes top 10
flist = stringr::str_sort(grep(
  glue("top10-goup_bar_vs_A_MxCre.vs.D_Csnk_cluster-{params$cluster}_p"),
  list.files(
    params$report_plots_folder_png,
    full.names=TRUE
  ),
...
fname=file.path(
  params$report_plots_folder_png,
  glue("go_down_genes_heatmap_vs_A_MxCre.vs.D_Csnk_cluster-{params$cluster}.png")
)
if(file.exists(fname)) knitr::include_graphics(fname)
```
...
### down genes top 10
flist = stringr::str_sort(grep(
  glue("top10-godown_bar_vs_A_MxCre.vs.D_Csnk_cluster-{params$cluster}_p"),
  list.files(
    params$report_plots_folder_png,
    full.names=TRUE
  ),
...
flist = stringr::str_sort(grep(
  glue("featureplot_top10_deg_vs_A_MxCre.vs.D_Csnk_cluster-{params$cluster}_id-"),
  list.files(
    params$report_plots_folder_png,
    full.names=TRUE
  ),
